...
//    _areaArray = [[NSArray alloc] initWithContentsOfFile:plistStr];
    if ([[NSUserDefaults standardUserDefaults]boolForKey:HAVECOUNTRYFILE]) {
        _areaArray = [NSKeyedUnarchiver unarchiveObjectWithFile:COUNTRYPATH];
    }
...
- (void)bl_show{
    ...
    [UIView animateWithDuration:animationDuration animations:^{
        CGRect tempRect = weakSelf.topView.frame;
        tempRect.origin.y = BL_ScreenH - topViewHeight - _pickViewFrame.size.height;
        weakSelf.topView.frame = tempRect;
        tempRect = _pickViewFrame;
        tempRect.origin.y = CGRectGetMaxY(weakSelf.topView.frame);
        weakSelf.pickView.frame = tempRect;
    }];
}
...
- (void)sureButtonClicked:(UIButton *)sender{
    ...
    CountryState *country = [_areaArray objectAtIndex:_provinceSelectedRow];
    States *state = [country.states objectAtIndex:_citySelectedRow];
    Cities *city= [state.cities objectAtIndex:_areaSelectedRow];
    
    _provinceId = (int)country.countryId;
    _stateId = (int)state.stateId;
    _cityId = (int)city.cityId;
    
    if (self.pickViewDelegate &&
        [self.pickViewDelegate respondsToSelector:@selector(bl_selectedAreaResultWithProvince:city:area:provinceId:stateId:cityId:)]) {
...
//            return [[[[[_areaArray objectAtIndex:_provinceSelectedRow] objectForKey:@"citylist" ] objectAtIndex:_citySelectedRow] objectForKey:@"arealist" ] count];
        {
            States *stae = [country.states objectAtIndex:_citySelectedRow];
            return stae.cities.count;
...
//            return [areaArr[row] objectForKey:@"areaName"];
            States *state = [sates objectAtIndex:_citySelectedRow];
            Cities *city = [state.cities objectAtIndex:row];
